...
function returnRes(AnsArray, ResArray, plist){
    ...
    for(oki = 0; oki < 15; oki++){
        if(AnsArray[oki] == difs[oki]){
            ResArray.push(<tr><td>{plist[oki]}</td><td style={{ backgroundColor :"mediumaquamarine"}} >{AnsArray[oki]}</td><td>{difs[oki]}</td></tr>);
        }
        else{
            ResArray.push(<tr ><td>{plist[oki]}</td><td style={{ backgroundColor :"salmon"}}>{AnsArray[oki]}</td><td>{difs[oki]}</td></tr>);
        }
    }
    return ResArray;
}
